# SaneSDD - Sane Spec Driven Development

A Claude Code plugin that orchestrates Claude through a phased, spec-driven development workflow. SaneSDD uses three channels: `.ssdd/specs/` for living documentation, `.ssdd/work/` for execution artifacts, and `.ssdd/design/` for domain-driven architecture. Each phase runs Claude as a different role (Product Manager, System Architect, Tech Lead, Developer, Story QA, Task QA) with role-specific instructions and tool restrictions.
...
Initialize a new SaneSDD project from within Claude Code:

```
/ssdd-init
```

This creates the required directory structure and generates an initial `.ssdd/INDEX.md`:
...
- **Spec channel** (`.ssdd/specs/`) — Living documentation describing the system as it currently exists. Organized as Theme → Feature → Story. Spec stories are promoted from the work channel after implementation completes.
- **Work channel** (`.ssdd/work/`) — Execution artifacts for planned changes. Organized as Epic → Story (with ACs) → Task. Work stories contain acceptance criteria and are the original source; they are kept as history after promotion.
- **Design channel** (`.ssdd/design/`) — Domain-driven architecture. Organized as Design → Domain → Component. Domains are bounded contexts grouping related components.
...
All commands are invoked from within a Claude Code session. SaneSDD auto-detects the project root by walking up from the current directory looking for a `.ssdd/` directory or `.git/` folder.
...
Initializes a new SaneSDD project by creating the required directory structure.
...
**What happens:**
1. Creates `.ssdd/specs/`, `.ssdd/work/`, and `.ssdd/design/` directories (skips any that already exist)
2. Generates an initial `INDEX.md`
3. For existing projects: surveys the codebase and generates design documentation:
   - `.ssdd/design/design.md` — system-wide architecture
   - `.ssdd/design/DOMAIN_NNN_slug/domain.md` — one per bounded context (breadth-first)
   - `.ssdd/design/DOMAIN_NNN_slug/COMP_<name>.md` — component docs within each domain
...
4. Produces `.ssdd/specs/THEME_NNN_slug/features/FEAT_NNN_slug/feature.md` with YAML frontmatter

**Output file example** (`.ssdd/specs/THEME_001_ecommerce/features/FEAT_001_checkout_resume/feature.md`):
```markdown
---
id: "FEAT_001"
...
**What happens:**
1. Looks up the feature in `.ssdd/specs/`
2. Creates a new epic (`.ssdd/work/EPIC_NNN_slug/`)
3. Claude discusses architecture interactively, then writes the design
4. Produces `.ssdd/work/EPIC_NNN_slug/high_level_design.md`
5. Creates or updates `.ssdd/design/DOMAIN_NNN_slug/COMP_<name>.md` for each component
6. Updates `.ssdd/design/design.md` with the system-wide architecture view
...
3. Produces `.ssdd/work/EPIC_NNN_slug/stories/STORY_NNN_slug/story.md` files

**Output file example** (`.ssdd/work/EPIC_001_checkout_resume/stories/STORY_001_save_cart/story.md`):
```markdown
---
id: "STORY_001"
title: "Save Cart"
...
Creates the `.ssdd/specs/`, `.ssdd/work/`, and `.ssdd/design/` directories. Only needed once per project. For existing projects, also generates design documentation (domains and components) from the codebase.

- **Inputs:** Existing codebase (optional), user-provided documentation (optional)
- **Outputs:** `.ssdd/specs/`, `.ssdd/work/`, `.ssdd/design/` directories, `INDEX.md`, and for existing projects: `.ssdd/design/design.md`, `.ssdd/design/DOMAIN_NNN_slug/domain.md`, `.ssdd/design/DOMAIN_NNN_slug/COMP_<name>.md`
- **Review:** For existing projects, review `.ssdd/design/design.md` for system-wide accuracy, and each `domain.md` for correct bounded context boundaries
...
- **Inputs:** User description of desired feature, existing `.ssdd/design/design.md` (if present)
- **Outputs:** `.ssdd/specs/THEME_NNN_slug/theme.md`, `.ssdd/specs/THEME_NNN_slug/features/FEAT_NNN_slug/feature.md`
...
- **Inputs:** `.ssdd/specs/THEME_NNN_slug/features/FEAT_NNN_slug/feature.md`, `.ssdd/design/design.md`, existing `.ssdd/design/DOMAIN_NNN_slug/domain.md` files
- **Outputs:** `.ssdd/work/EPIC_NNN_slug/epic.md`, `.ssdd/work/EPIC_NNN_slug/high_level_design.md`, `.ssdd/design/design.md` (updated), `.ssdd/design/DOMAIN_NNN_slug/domain.md` (new or updated), `.ssdd/design/DOMAIN_NNN_slug/COMP_<name>.md` (new or updated)
...
- **Inputs:** `.ssdd/specs/THEME_NNN_slug/features/FEAT_NNN_slug/feature.md`, `.ssdd/work/EPIC_NNN_slug/epic.md`, `.ssdd/work/EPIC_NNN_slug/high_level_design.md`
...
- **Inputs:** `.ssdd/work/EPIC_NNN_slug/development_plan.yaml`, all story and task files, `.ssdd/design/DOMAIN_NNN_slug/COMP_<name>.md`
- **Outputs:** Implementation code, updated task/story frontmatter statuses, `.ssdd/specs/THEME_NNN_slug/features/FEAT_NNN_slug/stories/STORY_NNN_slug.md` (promoted stories)
...
```
your-project/
├── .ssdd/                                    # SaneSDD artifact directory
...
│           └── ssdd-util.sh                    # Utility CLI wrapper
...
|-----------|---------|------------|
| `.ssdd/specs/` | Living documentation (themes, features, promoted stories) | Product Manager, Story QA (promotion) |
| `.ssdd/work/` | Epics, work stories, tasks, plans | System Architect, Tech Lead, Developer |
| `.ssdd/design/` | Domain-driven architecture (domains, components) | System Architect |
...
- **Agent context** is persisted in `.ssdd/work/EPIC_NNN_slug/agent/<role>/context.md`, allowing sub-agents to resume with full context across sessions.
...
The `ssdd-util` CLI provides deterministic state operations that the skills call via `"${CLAUDE_PLUGIN_ROOT}/scripts/ssdd-util.sh"`. You can also use it directly:

```bash
/path/to/plugins/ssdd/scripts/ssdd-util.sh --help
```
